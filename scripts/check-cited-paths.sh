...
#
# SCOPE, second half — THIS CHECK IS CROSS-WORKSPACE COUPLED, and that is a
# stronger caveat than "it is machine-local". Paths resolve against the disk
# this runs on, and the docs in this repo cite `~/.openclaw-nori/workspace/...`.
# So a green here is partly an assertion about ANOTHER AGENT'S filesystem: if
# Nori reorganises that tree, this repo goes red with no change in this repo,
# and the person who can fix it is not the person who will see the failure.
# Consequence, stated so nobody has to rediscover it: run this locally or as a
# pre-commit hook. Do NOT wire it into hosted CI, where every `~/`-rooted
# citation resolves against a runner that has none of these workspaces and the
# check goes red on all of them for no defect. A red here may be a fact about
# someone else's disk rather than about this repository.
#
# EXITS
...
#      that proves nothing. The suite's red arm keeps this honest across future
#      edits: `scripts/test-check-cited-paths.sh`, row G, which runs the actual
#      a224c71 revert from `fixtures/a224c71-README.md` and requires exit 1.
#
#      That sentence previously read "The suite's red arm (a224c71) is what
#      keeps this honest" while NO SUITE EXISTED — a header citing an artifact
#      that was not there, in the checker built to catch documents citing paths
#      that are not there. Coco blocked the merge on it. Left recorded rather
#      than quietly corrected, because the failure is the same one the tool
#      exists to prevent and the next person to write a forward reference here
